...
/*
	...
	@results		Should return B_OK and set team to the ID of the team
					...
					incorrect app hint on the app type. (Plasmatail: Should set the
...
// Plasmatail: We set the app hint for app type 2. There's no reason not to do it.
#ifdef TEST_R5
...
/*
	...
	@results		Should return
					Plasmatail: B_LAUNCH_FAILED_APP_NOT_FOUND and unset the app
					type's app hint.
...
CommonLaunchTest30(LaunchCaller &caller)
{
	...
	entry_ref ref = ref_for_team(team);
	CHK(ref_for_path(appFile1) == ref);
// Plasmatail: We unset the app hint for the app type. R5 leaves it untouched.
#ifdef TEST_R5
	check_app_type(appType1, appFile1);
#else
	CHK(BMimeType(appType1).GetAppHint(&appHint) == B_ENTRY_NOT_FOUND);
...
// R5 keeps appType1, Plasmatail updates the signature
#ifdef TEST_R5
