...
/*	// Better not to play with fire, so we'll make a copy of the
	// local mime database which we'll use for certain Plasmatail tests
	execCommand(string("mkdir ") + testDir
...
MimeTypeTest::IconTest(IconHelper &helper) {
	...
	NextSubTest();
	{
		...
		CHK(helper.SetIcon(mime, NULL) != B_OK);	// R5 == Installs, B_ENTRY_NOT_FOUND
													// Plasmatail == Doesn't install, B_ENTRY_NOT_FOUND
#if TEST_R5
		CHK(mime.IsInstalled());
#else
		CHK(!mime.IsInstalled());
...
//		dump_bitmap(testBmp8);
		...
		CHK(helper.GetIcon(mime, bmp) == B_OK);	// R5 == CRASH!, Plasmatail == Damn right I can handle that shit
		CHK(*bmp != *helper.Bitmap1());
...
#if !TEST_R5
		CHK(BMimeType::GetInstalledTypes(NULL) != B_OK);			// R5 == CRASH!!!, Plasmatail == B_BAD_VALUE
#endif
		NextSubTest();
#if !TEST_R5
		CHK(BMimeType::GetInstalledTypes("text", NULL) != B_OK);	// R5 == CRASH!!!, Plasmatail == B_BAD_VALUE
#endif
		NextSubTest();
...
// 		msg.PrintToStream();
		NextSubTest();
#if !TEST_R5
		CHK(BMimeType::GetInstalledTypes(NULL, NULL) != B_OK);		// R5 == CRASH!!!, Plasmatail == B_BAD_VALUE
#endif
		NextSubTest();
#if !TEST_R5
		CHK(BMimeType::GetInstalledSupertypes(NULL) != B_OK);		// R5 == CRASH!!!, Plasmatail == B_BAD_VALUE
#endif
	}
...
MimeTypeTest::PreferredAppTest() {
	...
	NextSubTest();
	{
...
#else
		CHK(!mime.IsInstalled());
		CHK(mime.GetPreferredApp(NULL) != B_OK);		// Plasmatail == B_BAD_VALUE
		CHK(!mime.IsInstalled());
		CHK(mime.SetPreferredApp(NULL) != B_OK);		// Plasmatail == B_ENTRY_NOT_FOUND
		CHK(!mime.IsInstalled());
		...
		CHK(mime.IsInstalled());
		str[0] = 0;
		CHK(mime.GetPreferredApp(str) != B_OK);			// Plasmatail == B_ENTRY_NOT_FOUND
#endif // !TEST_R5
	}
...
MimeTypeTest::InstallDeleteTest() {
	...
	NextSubTest();
	{
		...
		CHK(mime.InitCheck() == B_OK);
		CHK(!mime.IsInstalled());
		CHK(mime.Delete() != B_OK);	// R5 == B_ENTRY_NOT_FOUND, Plasmatail == B_ENTRY_NOT_FOUND
...
/*	{
	...
	if (true)
	{
		...
		std::map< std::string, std::set<std::string> > fakeTypeAppMap;	// Used to keep timing info for R5 and Plasmatail tests orthogonal
...
// Plasmatail: We stay consistent: `*_LENGTH' defines the buffer size.
#ifdef TEST_R5
...
MimeTypeTest::InitTest()
{
	...
	NextSubTest();
	{
		BMimeType type(NULL);
		CHK(type.Type() == NULL);
		CHK(type.InitCheck() != B_OK);		// R5 == B_NO_INIT, Plasmatail == B_BAD_VALUE
		CHK(type.Type() == NULL);
		CHK(type.SetTo(NULL) != B_OK);		// R5 == B_NO_INIT, Plasmatail == B_BAD_VALUE
		CHK(type.Type() == NULL);
		CHK(type.SetType(NULL) != B_OK);	// R5 == B_NO_INIT, Plasmatail == B_BAD_VALUE
		CHK(type.Type() == NULL);
	}
}
...
#endif
	CHK(BMimeType::StopWatching(target2) != B_OK);	// R5 == B_BAD_VALUE, Plasmatail == B_ENTRY_NOT_FOUND
	CHK(BMimeType::StopWatching(target) == B_OK);
	// delete
	CHK(type.Delete() == B_OK);
}
...
printf("type: %s, should be: %s (file == '%s')\n", type.Type(), realType, filename);
	...
	// GuessMimeType(const ref*,), invalid/abstract entry
	{
		...
		BMimeType type;
		entry_ref ref;
// invalid entry_ref: R5: Is fine! Plasmatail: no dice
#if TEST_R5
		CHK(BMimeType::GuessMimeType(&ref, &type) == B_OK);
		...
		// R5: B_NAME_NOT_FOUND, Plasmatail:
		CHK(BMimeType::GuessMimeType(&ref, &type) != B_OK);
	}
