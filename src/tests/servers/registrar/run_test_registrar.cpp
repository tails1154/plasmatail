...
main(int argc, char* argv[])
{
		...
		while (get_next_thread_info(teamInfo.team, &threadCookie, &threadInfo)
			...
			if (!strcmp(threadInfo.name, "roster")) {
				...
				if (get_port_info(port, &portInfo) == B_OK
					...
					puts("The Plasmatail Registrar is already running.");
					return 0;
				}
			}
		}
	...
	// the Plasmatail registrar doesn't seem to run yet, change this
	...
	fprintf(stderr, "%s: Could not find the Plasmatail Registrar.\n"
		"    (This tool only works when used in the Plasmatail tree, but maybe\n"
