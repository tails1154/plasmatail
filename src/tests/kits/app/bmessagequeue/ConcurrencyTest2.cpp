 ...
 void ConcurrencyTest2::TestThread2(void)
{
	...
	if (!unlockTest) {
		...
		// progress when a delete occurs.  The Plasmatail implementation
	...
	if (unlockTest) {
		...
		// The following test passes for the Plasmatail implementation but
		...
		// Plasmatail implementation detects that the message queue is deleted
		...
		// deleted.  The Plasmatail implementation will not emulate the Be
